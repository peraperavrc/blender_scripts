...
bpy.ops.mesh.primitive_cylinder_add(vertices=8, radius=1, depth=2, location=(0, 0, 0))
...
for i in range(8):
    angle = 2 * math.pi * i / 8
    parent_bone = armature.data.edit_bones.new(name=f"Bone.{i}")
    parent_bone.head = (math.cos(angle), math.sin(angle), 0)
    parent_bone.tail = (math.cos(angle), math.sin(angle), 0.5)
    for j in range(1, 4):
        child_bone = armature.data.edit_bones.new(name=f"Bone.{i}.{j}")
